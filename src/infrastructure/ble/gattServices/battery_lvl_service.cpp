...
String BatteryServiceHandler::readBatteryLevel(NimBLEClient* pClient) {
  ...
  if (pClient == nullptr) {
    LOG(LOG_GATT,"     Client not available");
    return batteryStr;
  }
  ...
  if (!pChar->canRead()) {
    LOG(LOG_GATT,"     Battery Level not readable");
    return batteryStr;
  }
  ...
  if (raw.empty() || raw.size() < 1) {
    LOG(LOG_GATT,"     Battery read failed (empty response)");
    return batteryStr;
  }
  ...
  if (level > 100) {
    LOG(LOG_GATT,"     Invalid battery value received");
    return batteryStr;
  }
